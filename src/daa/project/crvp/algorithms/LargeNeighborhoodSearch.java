	...
	private static CVRPSolution
		...
		for (int i = 0; i < numberOfRemovedClients; i++) {
			...
			while (clientToRemove == CVRPSolution.SEPARATOR || removedClients
			      .contains(clientToRemove) || checkRouteEmpty(indexToRemove,
			            previousVehiclesRoutes)) {
							indexToRemove =
							      rand.nextInt(previousVehiclesRoutes.size());
				clientToRemove = previousVehiclesRoutes.get(indexToRemove);
			}
			previousVehiclesRoutes
			      .remove(previousVehiclesRoutes.indexOf(clientToRemove));
